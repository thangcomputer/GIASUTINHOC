...
import { CheckCircle, Clock, UploadCloud, XCircle, ArrowRight, Award, FileText, File as FileIcon, Loader2 } from 'lucide-react';
...
export default function ExamPage() {
  ...
  const [examConfig, setExamConfig] = useState(null); // Từ db CourseExam
  
  const [quizScore, setQuizScore] = useState(0);
  const [essayAnswer, setEssayAnswer] = useState('');
  const [essayFile, setEssayFile] = useState(null);
  
  const [phase, setPhase] = useState('quiz'); // quiz -> essay -> submitted -> passed/failed
  const [currentQuiz, setCurrentQuiz] = useState(0);
  const [questions, setQuestions] = useState([]);
  const [examState, setExamState] = useState(null); // trạng thái DB
  ...
  useEffect(() => {
    ...
    // Fetch Course Config (CMS Admin defined questions & essay)
    fetch('/api/exams/course/' + id)
      ...
      .then(dConfig => {
        ...
        if (allQs.length === 0) {
          ...
          allQs = allQs.slice(0, 5); // Tối đa 5 câu fallback
        }
        
        setExamConfig({
           ...conf,
           questions: allQs,
           quizPassScore: conf.quizPassScore || Math.ceil(allQs.length * 0.5), // Mặc định phải đúng 50%
  ...
  const handleQuizAnswer = async (selectedIdx) => {
    ...
    if (currentQuiz < questions.length - 1) {
      ...
      // QUIZ FINISHED. Ký duyệt FAIL FAST.
      // Tính điểm quy đổi hệ 10
     ...
     try {
       const res = await fetch('/api/exams/submit', {
         ...
         body: JSON.stringify({
           ...
           essayFileUrl: fileUrl,  // Đã xử lý upload lên clouldinary (mock cho test)
       ...
       if (d.success) {
         setExamState(d.data); // Luôn lưu state mới
  ...
  const submitEssayPhase = async () => {
    ...
    // NẾU CÓ UPLOAD FILE, ĐIỀU HƯỚNG MOCK UPLOAD Ở ĐÂY
    let fileUrl = '';
    let fileName = '';
    if (essayFile) {
      // Giả lập lưu file - Backend thực tế dùng FormData & Multer -> Cloudinary
      // Tại app demo này dùng DataURI để pass tạm!
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', paddingTop: '80px', color: '#f1f5f9', paddingBottom: '100px' }}>
      {phase === 'passed' && <Confetti width={window.innerWidth} height={window.innerHeight} />}
      ...
      <div className="container" style={{ maxWidth: '800px', margin: '0 auto', padding: '0 20px' }}>
        ...
        {phase === 'quiz' && questions[currentQuiz] && (
          <div className="glass-card animate-fade-in-up" style={{ padding: '40px', background: 'linear-gradient(180deg, rgba(30,41,59,0.8), rgba(15,23,42,0.9))', borderTop: '4px solid #3b82f6' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
               <h3 style={{ margin: 0, color: '#38bdf8', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '1.3rem' }}>
                 TƯ DUY & LÝ THUYẾT 
               </h3>
               <div style={{ background: 'rgba(56,189,248,0.1)', padding: '6px 16px', borderRadius: '20px', color: '#38bdf8', fontWeight: 700 }}>
        ...
        {phase === 'essay' && (
          <div className="glass-card animate-fade-in-up" style={{ padding: '40px', background: 'linear-gradient(180deg, rgba(30,41,59,0.8), rgba(15,23,42,0.9))', borderTop: '4px solid #10b981' }}>
            <h3 style={{ margin: '0 0 20px', color: '#10b981', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '1.3rem' }}>
              <FileIcon size={24}/> THỰC HÀNH & KỸ NĂNG
            </h3>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <textarea
                ...
                placeholder="Nhập nội dung luận văn, tóm tắt, hoặc paste link Drive/Github của bạn vào đây..."
              ...
              <div style={{ marginBottom: '30px' }}>
                ...
                {filePreview && (
                  <div style={{ textAlign: 'center', marginTop: '10px' }}>
                    <button onClick={() => { setEssayFile(null); setFilePreview(null) }} style={{ background: 'none', border:'none', color: '#ef4444', cursor:'pointer', fontSize:'0.9rem'}}>Xóa tệp</button>
                  </div>
                )}
              </div>
            )}
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '30px', textAlign: 'right' }}>
              <button 
                ...
                {isSubmitting ? <Loader2 className="animate-spin" size={20}/> : <CheckCircle size={20}/>} NỘP BÀI THI
              </button>
            </div>
          </div>
        )}
        ...
        {phase === 'submitted' && (
          <div className="glass-card animate-fade-in-up" style={{ padding: '60px 40px', textAlign: 'center', background: 'linear-gradient(180deg, rgba(30,41,59,0.8), rgba(15,23,42,0.9))' }}>
            ...
            <div style={{ background: 'rgba(0,0,0,0.3)', padding: '24px', borderRadius: '16px', display: 'inline-block', textAlign: 'left', marginBottom: '30px' }}>
               <p style={{ margin: '0 0 10px', fontSize: '1.1rem' }}>✅ Bài Trắc nghiệm: <strong>{examState?.quizScore || Math.round((quizScore / questions.length) * 10)}/10 điểm</strong></p>
               <p style={{ margin: 0, fontSize: '1.1rem' }}>⏳ Bài Thực hành: <span style={{ color: '#f59e0b' }}>Giảng viên đang chấm điểm...</span></p>
            </div>
            <div>
...
fs.writeFileSync('src/pages/ExamPage.jsx', examPageJSX);
console.log('ExamPage overwritten with premium UI and fail fast upload capabilities!');
