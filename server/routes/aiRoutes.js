...
- Nếu vấn đề vượt phạm vi hoặc cần can thiệp trực tiếp: nêu rõ giới hạn và gợi ý học viên liên hệ đội ngũ Gia Sư Tin Học 24h / hỗ trợ trung tâm; không hứa hẹn cụ thể ngoài nền tảng.
...
function formatLessonContext(raw) {
  ...
  return `\n\n[NGỮ CẢNH BÀI HỌC — ưu tiên khi liên quan]\nHọc viên đang mở trang bài học trên Gia Sư Tin Học 24h. Dùng thông tin dưới để gợi ý sát module, thao tác và thuật ngữ đúng phạm vi (không bịa nội dung không có trong ngữ cảnh).\n${t}\n`;
}
...
Gia Sư Tin Học 24h — học tin học online (video, quiz, chat AI) hiện có các khóa học:
...
`.trim();
    ...
    const systemPrompt = `Bạn là Trợ lý Tư vấn Học Tập của trung tâm "Gia Sư Tin Học 24h". Nhiệm vụ DUY NHẤT của bạn là tư vấn về lộ trình học tập, khóa học và chương trình đào tạo của trung tâm.
